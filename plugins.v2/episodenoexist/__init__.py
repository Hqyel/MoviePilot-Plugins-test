...
class EpisodeNoExistInfo(TypedDict, total=False):
    # 季
    season: Optional[int]
...
class TvNoExistInfo(TypedDict):
    ...
    season_episode_no_exist_info: Dict[str, EpisodeNoExistInfo]
...
class EpisodeNoExist(_PluginBase):
    ...
    _clear: bool = False
    _scan_days: int = 0
    _force_scan_history: bool = False
    ...
    def init_plugin(self, config: dict[str, Any] | None = None):
        ...
        if config:
            ...
            self._force_scan_history = config.get("force_scan_history", False)
            ...
            _whitelist_media_servers = config.get(
                "whitelist_media_servers", ""
            )
            if _whitelist_media_servers and isinstance(
                _whitelist_media_servers, str
            ):
    ...
    def __get_mediaserver_tv_info(self) -> None:
        ...
        def __append_history(
            ...
            if item_unique_flag not in history["item_unique_flags"]:
            ...
            for library in librarys:
                ...
                for item in library_items:
                    ...
                    if item_unique_flag in item_unique_flags:
                        ...
                        should_rescan = False
                        detail = history["details"].get(item_unique_flag, {})
                        if detail:
                            tv_info = detail.get("tv_no_exist_info") or {}
                            tmdb_status = tv_info.get("status")
                            exist_status = detail.get("exist_status")
                                ...
                                if self._force_scan_history:
                                    ...
                                    if self._scan_days > 0 and last_air:
                                        try:
                                            ...
                                            if last_air_str != "未知":
                                                last_air_date = datetime.datetime.strptime(last_air_str, "%Y-%m-%d")
                                                ...
                                                if (now - last_air_date).days <= self._scan_days:
                                                    should_rescan = True
                                                    logger.info(f"【{item_title}】满足重扫条件 (连载/缺失 且 在时间范围内)，继续扫描")
                                                else:
                                                    logger.info(f"【{item_title}】虽连载/缺失但已超时 (超过 {self._scan_days} 天)，跳过")
                                        except Exception:
                                            pass
                                    elif self._scan_days == 0:
                                         should_rescan = True
                                         logger.info(f"【{item_title}】满足重扫条件 (连载/缺失 且 无时间限制)，继续扫描")
    ...
    def __update_config(self):
        ...
        __config = {
            ...
            "force_scan_history": self._force_scan_history,
            "clear": self._clear,
    ...
    def delete_history(self, key: str, apikey: str):
        ...
        is_success, historys = EpisodeNoExist.__remove_history_by_unique(
            historys, key
        )
    ...
    def __get_history_post_content(self, history: ExtendedHistoryDetail):
        def __count_seasons_episodes(
            seasons_episodes_info: Dict[str, EpisodeNoExistInfo],
        ):
